...
static uint64_t byte_swap64(uint64_t x)
{
...
//左移位
static uint32_t rol(const uint32_t value, const uint32_t nBit)
{
	return ((value << nBit) & 0xffffffff | ((value & 0xffffffff) >> (32 - nBit)));
}

static uint32_t P0(const uint32_t x)
{
	return (x ^ (rol(x, 9)) ^ (rol(x, 17)));
}
static uint32_t P1(const uint32_t x)
{
	return (x ^ (rol(x, 15)) ^ (rol(x, 23)));
}

static uint32_t FF0(const uint32_t X, const uint32_t Y, const uint32_t Z)
{
	return (X ^ Y ^ Z);
}
static uint32_t FF1(const uint32_t X, const uint32_t Y, const uint32_t Z)
{
	return ((X & Y) | (X & Z) | (Y & Z));
}

static int GG0(const uint32_t X, const uint32_t Y, const uint32_t Z)
{
	return (X ^ Y ^ Z);
}
static int GG1(const uint32_t X, const uint32_t Y, const uint32_t Z)
{
	return ((X & Y) | ((~X) & Z));
}
